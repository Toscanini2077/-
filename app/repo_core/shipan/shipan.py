...
<head>
    <style>
        ...
        __spHeader = "<div>{}年{}月{}日{}時{}分{}秒</div>".format(self.year,
                                                            self.month,
                                                            self.day,
                                                            self.hour,
                                                            self.minutes,
                                                            self.second)
        __spHeader = "{}<div>占事：{}</div>".format(__spHeader, self.占测的事)
        ...
        if self.昼占:
            __昼占 = "晝占"
        __spHeader = ("{}<div>月將：{}&nbsp&nbsp&nbsp月宿：{}&nbsp&nbsp&nbsp占時：{}&nbsp{}&nbsp&nbsp&nbsp"
                      "(空亡: {} {})</div>").format(
            __spHeader,
            self.yueJiang,
            self.moonMansion,
        ...
        __格局 = "<div><b>卦體:</b> {}</div>".format(" ".join(self.格局))
        __格局comment = ""
...
class SanChuan():
    ...
    def __涉害(self, ke):
        ...
        # 俱是季
        raise NoSanchuan('所臨皆四季，不能用涉害取三傳')

    def __遥克(self):
        if 干支(self.__四课.干, self.__四课.支) in self.bazhuan:
            raise NoSanchuan('八傳日不用遙克')
        ko = []
    ...
    def __昂星(self):
        ...
        for i in (self.__四课.一课, self.__四课.二课, self.__四课.三课, self.__四课.四课):
            if i[0] not in list_tmp:
                ...
                ko.append(i)
        if len(ko) != 4:
            raise NoSanchuan('課不備，不能用昂星取三傳')
        if self.__四课.干.属阳:
            chu = self.__天盘[支("酉")]
            zhong = self.__四课.支阳神
            mo = self.__四课.干阳神
            self.__type.append("虎視卦")
            return (chu, zhong, mo)
        else:
            chu = self.__天盘.临(支("酉"))
            zhong = self.__四课.干阳神
    ...
    def __别责(self):
        ...
        for i in (self.__四课.一课, self.__四课.二课, self.__四课.三课, self.__四课.四课):
            if i[0] not in list_tmp:
                ...
                ko.append(i)
        if len(ko) == 4:
            raise NoSanchuan('四課全備，不能用別責取三傳')
        if len(ko) != 3:
            raise NoSanchuan('用別責用於三課備取三傳')
        if self.__四课.干.属阳:
        ...
        zhong = self.__四课.干阳神
        mo = zhong
        self.__type.append("别責卦")
        return (chu, zhong, mo)

    def __八专(self):
        if 干支(self.__四课.干, self.__四课.支) not in self.bazhuan:
            raise NoSanchuan('不是八傳日')
        if self.__四课.干.属阳:
            chu = self.__四课.干阳神 + 2
            self.__type.append("八專卦")
    ...
    def __eq__(self, other):
        if not isinstance(other, SanChuan):
            raise ValueError('{0}不是三傳'.format(other))
...
class 天将盘():
    ...
    def __init__(self, t, s, 昼占=True):
        if not isinstance(t, TianPan):
            raise ValueError("{0} 不是天盤".format(t))
        if not isinstance(s, SiKe):
            raise ValueError("{0} 不是四課".format(s))
        self.__天盘 = t
        self.__四课 = s
